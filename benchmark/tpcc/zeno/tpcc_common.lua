...
function cmd_prepare()
   ...
   if sysbench.tid == 0 then
      print("Thread 0 create table finished\n")
   end
...
-- CUSTOMER TABLE
   ...
   query = string.format([[
	...
	c_credit kv_text, 
	c_credit_lim kv_int4, 
	c_discount kv_float4, 
	c_balance kv_float4, 
	c_ytd_payment kv_float4, 
	c_payment_cnt kv_int4, 
	c_delivery_cnt kv_int4, 
	c_data kv_text,
...
-- STOCK table
   ...
   query = string.format([[
      CREATE TRIGGER tri_update BEFORE UPDATE ON warehouse%d
      FOR EACH ROW EXECUTE PROCEDURE generic_delete_trigger(
  ...
  query = string.format([[
   CREATE TRIGGER tri_update BEFORE UPDATE ON district%d
   FOR EACH ROW EXECUTE PROCEDURE generic_delete_trigger(
...
query = string.format([[ 
   CREATE TRIGGER tri_update BEFORE UPDATE ON customer%d
   FOR EACH ROW EXECUTE PROCEDURE generic_delete_trigger(
...
query = string.format([[
   CREATE TRIGGER tri_update BEFORE UPDATE ON orders%d
   FOR EACH ROW EXECUTE PROCEDURE generic_delete_trigger(
...
query = string.format([[
   CREATE TRIGGER tri_update BEFORE UPDATE ON order_line%d
   FOR EACH ROW EXECUTE PROCEDURE generic_delete_trigger(
...
query = string.format([[
   CREATE TRIGGER tri_update BEFORE UPDATE ON stock%d
   FOR EACH ROW EXECUTE PROCEDURE generic_delete_trigger(
...
query = string.format([[
   CREATE TRIGGER tri_insert BEFORE INSERT ON warehouse%d
   FOR EACH ROW EXECUTE PROCEDURE generic_delete_trigger(
       w_id, w_name, w_street_1, w_street_2, w_city, w_state, 
       w_zip, w_tax, w_ytd
   );
...
query = string.format([[
CREATE TRIGGER tri_insert BEFORE INSERT ON district%d
FOR EACH ROW EXECUTE PROCEDURE generic_delete_trigger(
    d_id, d_w_id, d_name, d_street_1, d_street_2, d_city,
    d_state, d_zip, d_tax, d_ytd, d_next_o_id
);
...
query = string.format([[ 
CREATE TRIGGER tri_insert BEFORE INSERT ON customer%d
FOR EACH ROW EXECUTE PROCEDURE generic_delete_trigger(
   c_id, c_d_id, c_w_id, c_first, c_middle, c_last, 
   c_street_1, c_street_2, c_city, c_state, c_zip, 
   c_phone, c_since, c_credit, c_credit_lim, c_discount, 
...
query = string.format([[
CREATE TRIGGER tri_insert BEFORE INSERT ON orders%d
FOR EACH ROW EXECUTE PROCEDURE generic_delete_trigger(
    o_id, o_d_id, o_w_id, o_c_id, o_entry_d, 
    o_carrier_id, o_ol_cnt, o_all_local
);
...
query = string.format([[
CREATE TRIGGER tri_insert BEFORE INSERT ON order_line%d
FOR EACH ROW EXECUTE PROCEDURE generic_delete_trigger(
    ol_o_id, ol_d_id, ol_w_id, ol_number, ol_i_id, 
    ol_supply_w_id, ol_delivery_d, ol_quantity, 
    ol_amount, ol_dist_info
...
query = string.format([[
CREATE TRIGGER tri_insert BEFORE INSERT ON stock%d
FOR EACH ROW EXECUTE PROCEDURE generic_delete_trigger(
    s_i_id, s_w_id, s_quantity, s_dist_01, s_dist_02,
    s_dist_03, s_dist_04, s_dist_05, s_dist_06, s_dist_07,
    s_dist_08, s_dist_09, s_dist_10, s_ytd, s_order_cnt,
...
query = string.format([[
CREATE TRIGGER tri_insert BEFORE INSERT ON history%d
FOR EACH ROW EXECUTE PROCEDURE generic_delete_trigger(
...
query = string.format([[
CREATE TRIGGER tri_insert BEFORE INSERT ON item%d
FOR EACH ROW EXECUTE PROCEDURE generic_delete_trigger(
...
query = string.format([[
   CREATE TRIGGER tri_delete AFTER DELETE ON new_orders%d
   FOR EACH ROW EXECUTE PROCEDURE generic_delete_trigger(
...
con:query(query)
   ...
   for j = 1 , MAXITEMS do
      ...
      query = string.format([[(%d,%d,'%s'::kv_text,'%f'::kv_float4,'%s')]],
...
function load_tables(drv, con, warehouse_num)
   ...
   print(string.format("Thread %d is loading table DISTRICT&WAREHOUSE(wh=%d)", thread_id, warehouse_num))
    ...
    query = string.format([[(%d, '%s'::kv_text,'%s','%s', '%s', '%s', '%s', '%f'::kv_float4,'300000'::kv_float4)]],
   ...
   for d_id = 1 , DIST_PER_WARE do
      ...
      query = string.format([[(%d, %d, '%s','%s','%s', '%s', '%s', '%s', '%f'::kv_float4,'30000'::kv_float4,3001)]],
...
-- CUSTOMER TABLE
   ...
   for c_id = 1 , CUST_PER_DIST do
	...
	if c_id <= 1000 then
      ...
      query = string.format([[(%d, %d, %d, '%s'::kv_text,'OE','%s'::kv_text,'%s'::kv_text, '%s'::kv_text, '%s'::kv_text, '%s'::kv_text,'%s','%s',NOW(),'%s','50000'::kv_int4,'%f'::kv_float4,'-10'::kv_float4,'10'::kv_float4,'1'::kv_int4,'0'::kv_int4,'%s' )]],
...
-- HISTORY TABLE
   ...
   for c_id = 1 , CUST_PER_DIST do
      ...
      query = string.format([[(%d, %d, %d, %d, %d, NOW(), '10'::kv_float4, '%s' )]],
...
-- 3,000 rows in the ORDER table with
   a_counts[warehouse_num][d_id][o_id] = sysbench.rand.uniform(5,15)
      ...
      query = string.format([[(%d, %d, %d, %d, NOW(), %s, '%d'::kv_int4, '1'::kv_int4 )]],
...
-- STOCK table
   ...
   for s_id = 1 , MAXITEMS do
      ...
      query = string.format([[(%d, %d, '%d'::kv_int4,'%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','0'::kv_float4,'0'::kv_int4,'0'::kv_int4,'%s')]],
   ...
   for ol_id = 1, a_counts[warehouse_num][d_id][o_id] do 
      ...
      query = string.format([[(%d, %d, %d, %d, %d, %d, '%s'::timestamp, '5'::kv_int4, '%f'::kv_float4, '%s' )]],
