...
-- prep work
    ...
    local w_id = home_w_id
    local d_id = sysbench.rand.uniform(1, 10)
...
--                       :o_ol_cnt, :o_all_local);
  ...
  query = (([[INSERT INTO orders%d
                           (o_id, o_d_id, o_w_id, o_c_id,  o_entry_d, o_ol_cnt, o_all_local)
                    VALUES (%d,%d,%d,%d,NOW(),'%d'::kv_int4,'%d'::kv_int4)]]):
...
--	AND s_w_id = :ol_supply_w_id;*/
  ...
  query = (([[UPDATE stock%d
	                SET s_quantity = '%d'::kv_int4
	              WHERE s_i_id = %d 
		        AND s_w_id= %d]]):
...
--		:ol_dist_info);
  ...
  query = (([[INSERT INTO order_line%d
                                 (ol_o_id, ol_d_id, ol_w_id, ol_number, ol_i_id, ol_supply_w_id, ol_quantity, ol_amount, ol_dist_info)
	                  VALUES (%d, %d, %d, %d, %d, %d, '%d'::kv_int4, '%f'::kv_float4, '%s' )]]):
...
--  WHERE w_id =:w_id
  ...
  query = (([[UPDATE warehouse%d
	          SET w_ytd = w_ytd + '%f'::kv_float4 
...
--		AND d_id = :d_id;*/
  ...
  query = (([[UPDATE district%d 
                 SET d_ytd = d_ytd + '%f'::kv_float4 
               WHERE d_w_id = %d 
...
--	AND c_last = :c_last;*/
	...
	local namecnt = con:query_row(([[SELECT count(c_id) namecnt
			                  ...
			                  WHERE c_w_id = %d 
			                    AND c_d_id= %d
                                            AND c_last='%s'::kv_text]]):format(table_num, c_w_id, c_d_id, c_last ))
--		SELECT c_id
--		FROM customer
--		WHERE c_w_id = :c_w_id 
--		AND c_d_id = :c_d_id 
...
--		ORDER BY c_first;
	...
	rs = con:query(([[SELECT c_id
			   ...
			   WHERE c_w_id = %d AND c_d_id= %d
                             AND c_last='%s'::kv_text ORDER BY c_first]]
...
-- 	AND c_id = :c_id; */
    ...
    --			AND c_d_id = :c_d_id 
    --			AND c_id = :c_id
        query = (([[UPDATE customer%d
                        SET c_balance='%f'::kv_float4, c_ytd_payment='%f'::kv_float4, c_data='%s'
                      WHERE c_w_id = %d 
                        AND c_d_id=%d
                        AND c_id=%d]])
      ...
      con:query(query)
        ...
        query = (([[UPDATE customer%d
                        SET c_balance='%f'::kv_float4, c_ytd_payment='%f'::kv_float4 
                      WHERE c_w_id = %d 
                        AND c_d_id=%d
                        AND c_id=%d]])
...
--			       :h_amount, :h_data);*/
  ...
  query = (([[INSERT INTO history%d
                           (h_c_d_id, h_c_w_id, h_c_id, h_d_id,  h_w_id, h_date, h_amount, h_data)
                    VALUES (%d,%d,%d,%d,%d,NOW(),'%f'::kv_float4,'%s')]])
...
--				ol_w_id = :w_id;*/
        query = (([[UPDATE order_line%d 
                        SET ol_delivery_d = '%s'::timestamp 
                      WHERE ol_o_id = %d 
                        AND ol_d_id = %d 
                        AND ol_w_id = %d]])
                      :format(table_num, os.date("%Y-%m-%d %H:%M:%S"), no_o_id, d_id, w_id))
        con:query(query)
...
--        print(string.format("update customer table %d, cid %d, did %d, wid %d balance %f",table_num, o_c_id, d_id, w_id, sm_ol_amount))  				
        query = (([[UPDATE customer%d 
                        SET c_balance = c_balance + '%f'::kv_float4,
                            c_delivery_cnt = c_delivery_cnt + '1'::kv_int4
                      WHERE c_id = %d 
                        AND c_d_id = %d 
                        AND c_w_id = %d]])
...
--                      AND s_quantity < :level;*/
        ...
        query = (([[SELECT count(*) FROM stock%d
